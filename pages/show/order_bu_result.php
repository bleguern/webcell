...
function get_order_bu_list() {
	...
	if (isset($_SESSION['site_id']) && ($_SESSION['site_id'] != '')) {
		...
		$sql_query = 'SELECT UNIX_TIMESTAMP(custords_product.date) as custords_product_date, ' .
				     ...
				     'WHERE custords_product.direct_forwarding = 1 ';
...
<FORM NAME="resultForm" ACTION="order_bu_result.php" METHOD="POST">
<?php
start_loading();
$count = get_order_bu_list();
stop_loading();
?>
