...
function get_planning_board_cell_list() {
	...
	if ((isset($_REQUEST['id'])) && 
		...
		$sql_query .= 'AND cell.id = '.mysql_format_to_number($_REQUEST['id']).' ' .
		...
		if($result) {
			...
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
				...
				$result_table[$i][3] = $row['product_name'];
				$result_table[$i][4] = $row['flow_stock_dc'];
				$result_table[$i][5] = $row['flow_quantity_dc'];
				$result_table[$i][6] = $row['flow_date_dc'];
				$result_table[$i][7] = $row['flow_stock_bu'];
				$result_table[$i][8] = $row['flow_quantity_bu'];
				$result_table[$i][9] = $row['flow_date_bu'];
				$result_table[$i][10] = $quantity;
				$result_table[$i][11] = $row['product_minimum_stock'];
				$result_table[$i][12] = $to_produce;
				$result_table[$i][13] = $time;
				$result_table[$i][14] = $row['product_description'];
...
<FORM NAME="resultForm" ACTION="planning_board_by_cell_result.php" METHOD="POST">
<?php
start_loading();
if ((isset($_REQUEST['id'])) && ($_REQUEST['id'] != '')) {
	$count = get_planning_board_cell_list();
}
stop_loading();
?>
