...
function import_machine_time() {
	
	$fs = fopen($_FILES["machine_time"]["tmp_name"], 'r');
	...
	if ($fs) {
		...
		while (!feof($fs)) {
			...
			if (count($buffer) >= 2) {
				...
				$machine_time = str_replace(',', '.', trim($buffer[1]));
				
				if (is_numeric($machine_time)) {
					$sql_query = 'UPDATE product '.
								 'SET machine_time = '.mysql_format_to_number($machine_time).' ' .
...
<html>
<head>
	<title>ELBA - Appli Cellules - Importation des temps machine</title>
	<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
	<link href="../../css/style.css" rel="stylesheet" type="text/css">
</HEAD>
<BODY class="main_body">
<FORM NAME='importMachineTimeForm' ACTION='import_machine_time.php' METHOD='POST' enctype="multipart/form-data">
<TABLE class="normal">
	<TR>
		<TD class="main_title_text">Importation des temps machine</TD>
	</TR>
	<TR>
	...
	<TR>
		<TD align="center">
			<TABLE>
				<TR>
					<TD>
						<INPUT TYPE='file' NAME='machine_time'>
					</TD>
				</TR>
				<TR>
			...
			<TABLE>
				<TR>
					<TD>
						<INPUT TYPE='submit' NAME='import' VALUE='importation' ALT='Importation des temps machine'>
					</TD>
				</TR>
				<TR>
					<TD class="max_separator"></TD>
