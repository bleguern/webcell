...
function print_planning_board_by_cell() {
	...
	if ((isset($_SESSION['site_id'])) && 
		($_SESSION['site_id'] != '')) {
		$cell = mysql_simple_select_query('SELECT name FROM cell WHERE id = '.mysql_format_to_number($_REQUEST['id']));
									  
		$header = array(array('Machine', 15, "data"), 
						array('Reference', 20, "data"),
						array('Designation', 42, "data"),
		...
		if($result) {
			...
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
				...
				$data[$count] = array($row['machine_name'], 
		...
		print_pdf('Planning board : '.$_SESSION['site_name'].' : cellule '.$cell, $horizon, $_SERVER['REMOTE_ADDR'], $header, $data);
	}
}
